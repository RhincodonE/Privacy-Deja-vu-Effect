...
def compute_ntk_sample_pairs(model, stage1_loader, stage2_loader, top20_paths, device, output_csv):
    model.eval()

    # Pre-compute Stage 1 gradients for top20 samples
    grad_stage1 = {}
    idx = 0
    for imgs, _ in tqdm(stage1_loader, desc="Stage 1 Gradients"):
        batch_paths = [stage1_loader.dataset.samples[idx + i][0] for i in range(imgs.size(0))]
        for i, path in enumerate(batch_paths):
            if path in top20_paths:
                img = imgs[i].unsqueeze(0).to(device)
                model.zero_grad()
                output = model(img).sum()
                output.backward()
                grad = torch.cat([
                    p.grad.view(-1).cpu()
                    for p in model.parameters() if p.grad is not None
                ])
                grad_stage1[path] = grad
            idx += 1
    print(len(grad_stage1))


    # Compute NTK values directly without storing all stage-2 gradients
    results = []
    idx = 0
    for imgs, _ in tqdm(stage2_loader, desc="Stage 2 Gradients"):
        batch_paths = [stage2_loader.dataset.samples[idx + i][0] for i in range(imgs.size(0))]
        for i, path2 in enumerate(batch_paths):
            img = imgs[i].unsqueeze(0).to(device)
            model.zero_grad()
            output = model(img).sum()
            output.backward()
            ...
            # Immediately compute NTK values with stage-1 gradients
            for path1, grad1 in grad_stage1.items():
                ntk_value = torch.dot(grad1, grad2).item()
                results.append({
                    "Stage1_Path": path1,
                    "Stage2_Path": path2,
                    "NTK_Value": ntk_value
                })
            del grad2  # free memory after use
        idx += imgs.size(0)
    ...
    print(f"NTK pairwise results saved to {output_csv}")
...
def main(cfg):
    ...
    #specify the images in $X_k$ you want to compare with the $X_{k+1}$
    df = pd.read_csv('/results_subpop/result-new-0.4-instrumentality/comparison_target2_minus_target1.csv')
    top10_paths = df.nlargest(10, 'Delta')['Path'].tolist()
    bottom10_paths = ['/datasets/tiny-imagenet-200/train/n02481823/images/n02481823_216.JPEG','/tiny-imagenet-200/train/n03393912/images/n03393912_346.JPEG', 'etc.']
    top20_paths = top10_paths + bottom10_paths

    # right after you build dl1_tr and dl1_val:
    combined_samples = dl1_tr.dataset.samples + dl1_val.dataset.samples
    combined_dataset = CustomDataset(combined_samples,transform=dl1_tr.dataset.transform)

    dl1_tr_ntk = DataLoader(combined_dataset,batch_size=8,shuffle=False)
    dl2_tr_ntk = DataLoader(dl2_tr.dataset, batch_size=8, shuffle=False)

    ntk_sim_path = os.path.join(OUT_BASE, "ntk_mean_similarities")
    os.makedirs(ntk_sim_path, exist_ok=True)
    csv_path = os.path.join(ntk_sim_path,f"ntk_persample_sim_{cfg.superclass_name}_{cfg.random_seed}_{cfg.alpha:.2f}.csv")

    compute_ntk_sample_pairs(model, dl1_tr_ntk, dl2_tr_ntk, top20_paths, device, csv_path)
