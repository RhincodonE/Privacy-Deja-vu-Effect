...
def compute_ntk_similarity(model: torch.nn.Module,
                           ...
                           test: bool = False):
    """
    Compute NTK mean similarities between *old* and *new* IMDb samples for a
    BERT‑style model.

    Returns:
        np.ndarray of shape [N_old] (or [3] if test=True) containing
        ⟨∇_old,  mean(∇_new)⟩ for each old sample.
    """
    model.eval()
    max_count = 3 if test else None

    # ------------------------------------------------ 1) accumulate mean‑new‑grad
    g_new_acc = None
    count_new = 0

    for batch in tqdm(loader_new, desc="Accumulating new grads"):
        batch_size = batch["input_ids"].size(0)
        for i in range(batch_size):
            # build single‑example dict
            single = {k: v[i:i+1].to(device, non_blocking=True)
                      for k, v in batch.items()
                      if k not in ("labels", "path")}  # labels not needed

            model.zero_grad()
            out = model(**single).logits.sum()
            out.backward()
            ...
            g_new_acc = g_vec.clone() if g_new_acc is None else g_new_acc + g_vec
            count_new += 1
            if max_count and count_new >= max_count:
                break
        if max_count and count_new >= max_count:
            break
    ...
    # ------------------------------------------------ 2) dot product for each old sample
    sims       = []
    count_old  = 0
    max_old    = 3 if test else None

    for batch in tqdm(loader_old, desc="Dot with mean‑new‑grad"):
        batch_size = batch["input_ids"].size(0)
        for i in range(batch_size):
            single = {k: v[i:i+1].to(device, non_blocking=True)
                      for k, v in batch.items()
            ...
            count_old += 1
            if max_old and count_old >= max_old:
                break
        if max_old and count_old >= max_old:
            break
...
def main(cfg):
    ...
    dl1_tr_ntk = DataLoader(dl1_tr.dataset, batch_size=20, shuffle=False)
    dl2_tr_ntk = DataLoader(dl2_tr.dataset, batch_size=20, shuffle=False)
