...
submit_target_job() {
  local SPLIT_SEED=$1
  local job_out
  job_out=$(sbatch --gres=gpu:1 --cluster=chip-gpu --mem=1000 --time=3:00:00 --chdir=$(pwd) --output=./logs/target_model_${SPLIT_SEED}_%j.out --error=./logs/target_model_${SPLIT_SEED}_%j.err <<EOF
#!/bin/bash
...
python Fine-tune-vit.py --superclass_name instrumentality --alpha ${ALPHA} --SGD_New --random_seed ${RANDOM_SEED_TARGET} --split_seed ${SPLIT_SEED}
EOF
)
